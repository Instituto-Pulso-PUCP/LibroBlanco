...
def run():
    ...
    name_to_orcid = {r.name_norm: str(r.orcid).strip() for r in inv.itertuples()
    ...
    for p in proj.itertuples():
        ...
        for a in rows.itertuples():
            ...
            cand.append({'project_id':p.project_id,'codigo_campus':p.codigo_campus,'project_year':p.year,'project_title':p.title,'coordinator_original':p.coordinator_original,'coordinator_orcid':name_to_orcid.get(p.coordinator_norm, ''),'publication_id':int(a.publication_id),'publication_year':y,'publication_title':pub.get('title',''),'doi':pub.get('doi',''),'source_RI':pub.get('RI',0),'source_SCOPUS':pub.get('SCOPUS',0),'source_WOS':pub.get('WOS',0),'matched_author':a.author_name_raw,'year_delta':delta,'title_similarity':sim,'score':round(score,2),'confidence':'Alta' if score>=85 else ('Media' if score>=65 else 'Baja'),'evidence':' | '.join(ev)})
